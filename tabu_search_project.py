...
def simulatedAnnealingAlgo(graph, numberOfCities, epochs, same_first_sol, graphShow):
    times=[]
    start = time.time()
...
def generate_first_solution(graph):
...
def main():
    ...
    numberOfEpochsSimu = int(sys.argv[5])
    ...
    simulatedBest, times_simu = simulatedAnnealingAlgo(graph, graph.nb_nodes, numberOfEpochsSimu,same_first_sol, graphShow)

    # Tabu Search Algorithm
    first_solution, distance_of_first_solution = generate_first_solution(graph)
    cost_list.append(distance_of_first_solution)

    print("init sol ", first_solution)
    ...
    if graphShow:
        ...
        compare2Algorithms(cost_list, simulatedBest, epochs_list)
        compare2Algorithms(cost_list, simulatedBest, time_line_tabu, times_simu)
