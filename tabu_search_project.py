...
def changePosition(currentSolution):
    # save the first and the last indexes with zero
    # so reduce by 2 the current solution length

    sizeCurrentSol = len(currentSolution)
    highestOptionalVal = sizeCurrentSol - 2
    a = random.randint(1, highestOptionalVal)
    b = random.randint(1, highestOptionalVal)
    while b == a:
        b = random.randint(1, highestOptionalVal)
...
def simulatedAnnealingAlgo(graph, same_first_sol, graphShow):
    times = []
    start = time.time()
    ...
    finalBestScore = listBestScore[-1]
    return currentSolution, listBestScore, times, finalBestScore
...
def main():
    ...
    simulatedBest, listBestscores, times_simu, finalBestScore =\
        simulatedAnnealingAlgo(graph, same_first_sol, graphShow)
    print(f"Best solution of Simulated annealing: {simulatedBest}, \n with total cost: {finalBestScore}")
